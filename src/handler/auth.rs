...
use axum::{
    extract::Query, http::{header, response, HeaderMap, StatusCode}, response::{IntoResponse, Redirect}, routing::{get, post}, Extension, Json, Router
};
...
use crate::{
    db::UserExt,
    dtos::{
        self, ForgotPasswordRequestDto, LoginUserDto, RegisterUserDto, ResetPasswordRequestDto,
...
pub async fn register(
    ...
    match result {
        Ok(_user) => {
            ...
            if let Err(e) = send_email_request {
            ...
            Ok((
                ...
                Json(dtos::Response {
                    status: "success",
                    message: "Registration successfull, please check your mailbox for verification"
                        .to_string(),
                }),
            ))
        }
        ...
        // when you write Err(withYourOwnErrType), you need to add another Err below for any other error
...
pub async fn verify_email(
    ...
    if let Some(expires_at) = user.token_expires_at {
        if Utc::now() > expires_at {
            return Err(HttpError::unauthorized(
                "Verification token is expired".to_string(),
            ));
        } else {
    ...
    let mut headers = HeaderMap::new();
    headers.append(header::SET_COOKIE, cookie.to_string().parse().unwrap());
    let frontend_url = format!("http://localhost:3000/");
...
pub async fn forgot_password(
    ...
    let email_send = send_forgot_password_email(&user.email, &user.name, &reset_link).await;

    if let Err(e) = email_send {
